...
@section('content')
    <div class="container-fluid">
        <h3 class="text-center my-4">تقرير حركة تسكين الغرف <small>(مغادرين)</small></h3>
        <div class="col-md-12 mb-2 unprint">
            <form action="{{ route('rooms.status') }}" method="GET" role="search">
                <div class="row">
                    ...
                    <table class="table table-bordered table-hover my-3">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center">تاريخ المغادرة</th>
                                <th class="text-center">المبلغ</th>
                                <th class="text-center">السياحة</th>
                            ...
                            @if (count($myA))
                                @foreach ($myA as $bill)
                                    <tr class='clickable-row' data-href='{{ route('bill.show.trashed', $bill['bill_id']) }}'>
                                        ...
                                        <td>{{ $bill['arrDate'] }}</td>
                                        <td>{{ $bill['leaDate'] }}</td>
                                        <td hidden>{{ $sp += $bill['price'] }}</td>
                                        <td hidden>{{ $stu += $bill['tourism'] }}</td>
                                        <td hidden>{{ $st += $bill['tax'] }}</td>
                                        <td hidden>{{ $tot += $bill['price'] + $bill['tourism'] + $bill['tax'] }}</td>
                                        <td>{{ $bill['price'] }}</td>
                                        <td>{{ $bill['tourism'] }}</td>
                                        <td>{{ $bill['tax'] }}</td>
                                        <td>{{ $bill['price'] + $bill['tourism'] + $bill['tax'] }}</td>
                                    </tr>
                                @endforeach
                            @endif
                            <tr>
                                ...
                                <td>المجموع</td>
                                <td colspan="5" class="text-center">عدد السجلات{{ $rec }}</td>
                                <td class="unprint"></td>
                                <td>{{ $sp }}</td>
                                <td>{{ $stu }}</td>
                                <td>{{ $st }}</td>
